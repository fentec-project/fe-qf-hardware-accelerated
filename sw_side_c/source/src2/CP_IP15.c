...
/**************************************************************************************************************/
/**************************************************************************************************************/
int fpga_cp_ip15(u8 cmd_cp, u8 L1_param,  u8 L2_param, u8 *input, u32 length, u8 *output)
{
	int Status;

	u32 command = (cmd_cp & 0x0000000F) | (((length / 8) << 4) & 0x00000FF0) |
					((L1_param << 12) & 0x0000F000) | ((L2_param << 16) & 0x000F0000);
	Xil_Out32(FPGA_CP_IP15_CMD, command);

	//-------------------------------------------------------------------------------------------------
	if ((cmd_cp == CP_IMEM_LD) || (cmd_cp == CP_L3L1_DMEM_WR))
	{
		Status = axi_dma_send_packet(TRUE, AXI_DMA7_CP1415, AXI_DMA7_CP1415_DEV_ID, length, input);
	...
	{
		Status = axi_dma_receive_packet(TRUE, AXI_DMA7_CP1415, AXI_DMA7_CP1415_DEV_ID, length, output);
		if (Status != XST_SUCCESS)
		{
	...
	//-------------------------------------------------------------------------------------------------
	while((Xil_In32(FPGA_CP_IPs_STATUS) & 0xC0000000) != 0xC0000000); // FPGA_CP_IP0 busy checking!
	Xil_Out32(FPGA_CP_IP15_CMD, 0x00000000);
...
void test_cp_ip15()
{
	...
	//------------------------------------------------------------------------------------------
	xil_printf("\r\n");
	xil_printf("----- Start of fpga_cp_ip15() -----\r\n");
	...
	fpga_cp_ip15(CP_L3L1_DMEM_WR, X0_L1_PARAM, NO_PARAM, DMEM_IN_X0, L3L1_PARAM_WR_LEN, NULL);
	fpga_cp_ip15(CP_L3L1_DMEM_WR, X1_L1_PARAM, NO_PARAM, DMEM_IN_X1, L3L1_PARAM_WR_LEN, NULL);
	...
	fpga_cp_ip15(CP_IMEM_LD, NO_PARAM, NO_PARAM, Micro_Code_Prog_MMM, PROG_LD_LEN, NULL);
	...
	fpga_cp_ip15(CP_L3L1_DMEM_RD, X2_L1_PARAM, NO_PARAM, NULL, L3L1_PARAM_RD_LEN, output_packet);
	...
	// CP_IP input parameters writing on DMEM //////////////////////////////////////////
	fpga_cp_ip15(CP_L3L1_DMEM_WR, X0_L1_PARAM, NO_PARAM, DMEM_IN_X0, L3L1_PARAM_WR_LEN, NULL);
	fpga_cp_ip15(CP_L3L1_DMEM_WR, X2_L1_PARAM, NO_PARAM, DMEM_IN_X2, L3L1_PARAM_WR_LEN, NULL);

	// CP_IP Micro_Code_Prog_ME loading on IMEM (X1 = (X0 ^ X2) mod n2) ////////////////
	// Sub-operations: Mapping X0; X1R = (X0R ^ X2) mod n2, Un-mapping X1R to X1
	fpga_cp_ip15(CP_IMEM_LD, NO_PARAM, NO_PARAM, Micro_Code_Prog_ME, PROG_LD_LEN, NULL);

	// CP_IP Micro_Code_Prog_ME running ////////////////////////////////////////////////
	fpga_cp_ip15(CP_PROG_RUN, NO_PARAM, NO_PARAM, NULL, 0, NULL);

	// CP_IP output parameter reading from DMEM ////////////////////////////////////////
	fpga_cp_ip15(CP_L3L1_DMEM_RD, X1_L1_PARAM, NO_PARAM, NULL, L3L1_PARAM_RD_LEN, output_packet);
	...
	//------------------------------------------------------------------------------------------
	fpga_cp_ip15(CP_L1L2_DMEM_WR, X1_L1_PARAM, Y3_L2_PARAM, NULL, L1L2_PARAM_WR_LEN, NULL);
	fpga_cp_ip15(CP_L1L2_DMEM_RD, X5_L1_PARAM, Y3_L2_PARAM, NULL, L1L2_PARAM_RD_LEN, NULL);

	// CP_IP output parameter reading from DMEM ////////////////////////////////////////
	fpga_cp_ip15(CP_L3L1_DMEM_RD, X5_L1_PARAM, NO_PARAM, NULL, L3L1_PARAM_RD_LEN, output_packet);
	...
	//------------------------------------------------------------------------------------------
	xil_printf("\r\n");
	xil_printf("-----  End  of fpga_cp_ip15() -----\r\n");
}
